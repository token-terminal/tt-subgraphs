...
import { Transfer, AaveTokenV2 } from '../generated/AaveTokenV2/AaveTokenV2'
import { TokenHolder, TransferEvent } from '../generated/schema'
import { getOrCreateToken, getOrCreateGovernanceToken, createTokenHolder, correctForDecimals } from './helpers'
...
function updateTokenHolders(
  ...
  if (tokenHolder == null) {
    ...
    governanceToken.numberOfTokenHolders = governanceToken.numberOfTokenHolders.plus(ONE_BI)
  }
  ...
  tokenHolder.balance = increase ? tokenHolder.balance.plus(valueNative) : tokenHolder.balance.minus(valueNative)
  ...
  if (tokenHolder.balance == ZERO_BD) {
    ...
    governanceToken.numberOfTokenHolders = governanceToken.numberOfTokenHolders.minus(ONE_BI)
  } else {
    tokenHolder.save()
  }
...
function updateTransferEvents(event: Transfer): void {
  ...
  let contract = AaveTokenV2.bind(Address.fromString(contractAddress))
  ...
  let transferAmountNative = correctForDecimals(event.params.value, token.decimals)
  let fromBalanceNative = correctForDecimals(contract.balanceOf(fromAddress), token.decimals)
  let toBalanceNative = correctForDecimals(contract.balanceOf(toAddress), token.decimals)
