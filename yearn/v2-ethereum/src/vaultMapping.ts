...
import { NewVault, NewExperimentalVault } from '../generated/Registry7/Registry'
import { Vault as VaultTemplate, Strategy as StrategyTemplate } from '../generated/templates'
import {
  Vault,
  StrategyAdded,
  StrategyAdded1,
...
export function handleStrategyReported(event: StrategyReported1): void {
  ...
  if (!tryPricePerShare.reverted && !tryTotalAssets.reverted && !tryTotalDebt.reverted) {
    vault.totalDebt = amountToDenomination(tryTotalDebt.value, token.decimals).div(
      amountToDenomination(tryPricePerShare.value, token.decimals)
    )
    vault.totalAssets = amountToDenomination(tryTotalAssets.value, token.decimals).div(
      amountToDenomination(tryPricePerShare.value, token.decimals)
    )
  }
